    ...
    public class Program
    {
        ...
        public static void Receiver()
        {
            ...
            try
            {
                ...
                while (true)
                {
                    byte[] receiveBites = receivingUdpClient.Receive(
                        ref RemoteIpEndPoint);

                    string returnData = Encoding.UTF8.GetString(receiveBites);

                    Console.ForegroundColor = ConsoleColor.Green;
                    Console.WriteLine("Поулчил команду: " + returnData.ToString());

                    if (returnData.ToString().Contains("/start"))
                    {
                        string[] dataMessage = returnData.ToString().Split('|');
                        ViewModelUserSettings viewModelUserSettings = new JsonConvert.DeserializedOblect<ViewModelUserSettings>(dataMessage[1]);
                        Console.ForegroundColor = ConsoleColor.Green;
                        Console.WriteLine("Покдлючился пользователь: {viewModelUserSettings.IPAdress} : {viewModelUserSettings.Port}");
                        remoteIPAdress.Add(viewModelUserSettings);
                    ...
                    else
                    {
                        ...
                        ViewModelUserSettings viewModelUserSettings = JsonConvert.DeserializedOblect<ViewModelUserSettings>(dataMessage[1]);
            ...
            while (true)
            {
                ...
                if (RemoteSnakes.Count > 0)
                {
                    foreach (ViewModelGames DeadSnake in RemoteSnakes)
                    {
                        ...
                        remoteIPAddress.RemoveAll(x => x.IdSnakePlayers.GameOver);
                    }
                    foreach (ViewModelUserSettings User in remoteIPAddress)
                    {
                        Snakes Snake = viewModelGames.Find(x => x.IdSnake == User.IdSnake).SnakesPlayers;

                        for (int i = Snake.Points.Count; i > 0; i--)
                        {
                            if (i != 0)
                            {
                        ...
                        {
                            for (int i = 1; i < Snake.Points.Count)
                            {
